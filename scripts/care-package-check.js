...
if(rules.uraniumChance>.1)fail('care packages would flood the scarce Uranium economy');
if(rules.dealerBoostChance!==.04)fail('the free five-minute Dealer jackpot must remain a rare 4% roll');
const expectedHourlyBoosts=3600000/((rules.minimumIntervalMs+rules.maximumIntervalMs)/2)*rules.dealerBoostChance;
if(expectedHourlyBoosts<1||expectedHourlyBoosts>1.3)fail(`Dealer jackpots drifted to ${expectedHourlyBoosts.toFixed(2)} expected activations/hour`);
...
for(const marker of ['app.css?build=33','js/core/config.js?build=23','js/core/economy.js?build=2','js/core/state.js?build=19','js/systems/operations.js?build=1','js/systems/merchant.js?build=4','js/systems/care-package.js?build=1','js/audio.js?build=13'])if(!html.includes(marker))fail(`cache/build marker missing: ${marker}`);
