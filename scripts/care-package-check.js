...
const rules=sandbox.window.BunkrConfig?.CARE_PACKAGE;
...
for(const [pattern,message] of [
  ...
  [/bunkr:care-package-landed/,'landing event is missing'],
  [/bunkr:care-package-opened/,'opening reward event is missing'],
...
if(!/function activateFree\(id,source='care-package'\)/.test(merchant)||!/bunkr:merchant-free-activated/.test(merchant))fail('Merchant needs a no-cost, immediate activation API');
...
if(!/bunkr:care-package-landed',carePackageLandSound/.test(audio)||!/bunkr:care-package-opened',carePackageOpenSound/.test(audio))fail('landing and reward events need dedicated audio');
for(const marker of ['app.css?build=61','js/core/config.js?build=38','js/core/economy.js?build=3','js/core/state.js?build=28','js/systems/operations.js?build=3','js/systems/merchant.js?build=5','js/systems/care-package.js?build=3','js/audio.js?build=19'])if(!html.includes(marker))fail(`cache/build marker missing: ${marker}`);
...
console.log('Bunkr care-package balance passed: 90–150s drops, five-second claims, responsive alpha assets, scarce jackpots and unified rewards.');
