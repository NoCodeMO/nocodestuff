...
for(const [pattern,message] of [
  ...
  [/matchMedia\('\(orientation:portrait\) and \(max-width:699px\), \(orientation:landscape\) and \(min-width:560px\) and \(max-height:600px\)'\)/,'phone visibility must follow both portrait and landscape layouts'],
...
for(const [pattern,message] of [
  ...
  [/html\.standalone-app #scene\{bottom:calc\(66px \+ env\(safe-area-inset-bottom\)\)!important\}/,'portrait combat controls must clear the standalone safe area'],
  [/#app:not\(\.bunkerOpen\) #bunker\{display:none\}#app:not\(\.bunkerOpen\) #scene\{right:0!important\}/,'landscape combat must use the full screen until BUNKER opens'],
