...
if(common.idleAsset!=='assets/enemy-common-drifter-idle.png')fail('The Drifter must expose its approved three-frame idle sheet');
...
if(common.hitAsset!=='assets/enemy-common-drifter-hit.png')fail('The Drifter must expose its approved three-frame hit sheet');
const hitAsset=path.join(root,common.hitAsset);if(!fs.existsSync(hitAsset))fail(`missing hit sprite ${common.hitAsset}`);
const hitBuffer=fs.readFileSync(hitAsset);if(!hitBuffer.subarray(1,4).equals(Buffer.from('PNG'))||hitBuffer[25]!==6)fail('The Drifter hit sheet must be a real RGBA PNG with transparent alpha');
if(hitBuffer.readUInt32BE(16)!==1881||hitBuffer.readUInt32BE(20)!==836)fail('The Drifter hit sheet must contain three equal 627x836 cells');
...
for(const [pattern,message] of [
  ...
  ,[/const baseDamage=tapDamage\(\),critical=criticalHit\(\),criticalMultiplier=critical\?COMBAT\.criticalMultiplier\*\(window\.BunkrPrestige\?\.criticalMultiplier\?\.\(\)\|\|1\):1,encounterMultiplier=.*?,damage=baseDamage\*criticalMultiplier\*encounterMultiplier/,'critical damage, including Elara, must be calculated once in core combat']
...
if(!/function enemyImpactPoint\(detail=\{\}\)/.test(visuals)||!/enemyStack\.querySelectorAll\('\.enemySprite'\)/.test(visuals)||!/function positionImpact\(point\)/.test(visuals))fail('shot impact must track a rendered zombie sprite instead of its oversized layout container');
