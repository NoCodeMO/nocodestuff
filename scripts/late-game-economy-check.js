...
const earlyWorkshop=economy.roomCostAt('workshop',62,1),expectedEarly=Math.floor(cfg.ROOMS.workshop.base*Math.pow(cfg.ROOM_ECONOMY.costGrowth,62));
assert(earlyWorkshop===expectedEarly,'the existing early-game room curve changed before its quadrillion transition');
const lateGenerator=economy.roomCostAt('generator',1979,.6),nextGenerator=economy.roomCostAt('generator',1980,.6);
assert(Number.isFinite(lateGenerator)&&lateGenerator>1e140,'level 1979 must have a real late-game price instead of the old 9.01Qa clamp');
assert(nextGenerator>lateGenerator&&Math.abs(nextGenerator/lateGenerator-cfg.ROOM_ECONOMY.lateCostGrowth)<1e-12,'late-game prices must keep growing at the configured rate');
assert(lateGenerator!==Number.MAX_SAFE_INTEGER,'room prices may never clamp to Number.MAX_SAFE_INTEGER again');
const screenshotCoins=5.63e152,firstMax=economy.affordableRoomLevels('generator',screenshotCoins,1979,.6,500);
assert(firstMax.count>0&&firstMax.count<500,'the reported level-1979 save must receive a bounded MAX quote');
assert(economy.validPurchase(firstMax,screenshotCoins,1979),'the bounded MAX quote must be transaction-safe');
...
assert(html.indexOf('js/core/economy.js?build=1')<html.indexOf('js/core/state.js?build=17'),'the safe economy module must load before save-state normalization');
console.log(`Afterlight late-game economy passed: LV 1979 costs ${lateGenerator.toExponential(3)}, MAX buys ${firstMax.count} level(s), and repeated MAX is blocked.`);
