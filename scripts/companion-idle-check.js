...
const pets=sandbox.window.AfterlightConfig.EXPEDITION_COMPANIONS;
function webpSize(buffer){
  ...
  for(let offset=12;offset+8<=buffer.length;){const type=buffer.toString('ascii',offset,offset+4),size=buffer.readUInt32LE(offset+4),data=offset+8;
    ...
    if(type==='VP8L'){const bits=buffer.readUInt32LE(data+1);return[1+(bits&0x3fff),1+((bits>>14)&0x3fff)]}
...
for(const pet of pets){const file=path.join(root,pet.asset);assert(fs.existsSync(file),`${pet.id} sprite is missing`);const buffer=fs.readFileSync(file),[width,height]=webpSize(buffer);assert(width===768&&height===256,`${pet.id} must be a 768x256 three-frame strip, got ${width}x${height}`);assert(buffer.length>80000,`${pet.id} sprite looks unexpectedly empty`)}
...
for(const [pattern,message] of [[/className='companionUnit hidden'/,'combat companion unit is missing'],[/function applyCompanion\(/,'companion renderer is missing'],[/afterlight:companion-selected/,'companion selection event is not connected'],[/COMPANIONS\.map\(item=>item\.asset\)/,'companion assets are not preloaded']])assert(pattern.test(visuals),message);
for(const [pattern,message] of [[/selectedCompanion:null/,'selected companion state is missing'],[/state\.schema=20/,'companion selection migration needs schema 20'],[/companionIds=new Set/,'old companion saves must be validated']])assert(pattern.test(state),message);
