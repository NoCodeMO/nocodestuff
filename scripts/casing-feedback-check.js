...
for(const [pattern,message] of [[/CASING_AUDIO_SRC='assets\/audio\/casing-drop-real\.wav'/,'audio must load the real casing recording'],[/fetch\(CASING_AUDIO_SRC,\{cache:'force-cache'\}\)/,'recording must preload before the first shot'],[/decodeAudioData\(bytes\.slice\(0\)\)/,'recording must decode through the unlocked Web Audio context'],[/function casingSound\(\)/,'metal casing sound is missing'],[/now-lastCasing<70/,'casing sound needs its own spam limiter'],[/level=effectsEnabled\?volumes\.combat:0/,'casing sound must follow the combat volume channel'],[/CASING_VOLUME=\.75/,'casing extra must stay exactly 25% below its original level'],[/gains\[variant\]\*CASING_VOLUME\*level/,'casing attenuation must apply before combat-channel scaling'],[/while\(casingVoices\.length>=6\)/,'overlapping casing sounds need a hard voice cap'],[/startDelay=Math\.max\(0,\.16-elapsed\)/,'real impact must remain synchronized with the visual landing'],[/playbackRate\.value=rates\[variant\]/,'repeated shots need natural pitch variation'],[/casingStatus:\(\)=>\(\{loaded:/,'browser smoke test needs decoded sample diagnostics'],[/window\.addEventListener\('afterlight:shot',casingSound\)/,'casing sound must use the shot event']])assert(pattern.test(audio),message);
...
for(const marker of ['app.css?build=58&pets=1&casing=1','js/ui/visuals.js?build=29&casing=1','js/audio.js?build=17&casing=1&realcasing=2'])assert(html.includes(marker),`cache marker missing: ${marker}`);
...
console.log('Afterlight casing feedback passed: untouched sprite/trajectory plus a verified real metal impact, two bounces, mobile voice cap and combat-volume routing.');
