...
for(const [pattern,message] of [[/schema:10/,'Save schema must migrate to version 10'],[/command:\{read:\[\],claimed:\[\],lastTab:'guide'/,'Command Center state defaults are missing'],[/uiSfx:true,reducedEffects:false/,'Settings defaults are missing']])assert(pattern.test(state),message);
