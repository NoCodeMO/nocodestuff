...
const context={window:{}};vm.runInNewContext(read('js/core/config.js'),context,{filename:'config.js'});const cfg=context.window.BunkrConfig,p=cfg.PRESTIGE,skins=cfg.SURVIVOR_SKINS.filter(skin=>skin.unlock?.type==='prestige');
...
assert(/BunkrPrestige\?\.resourceMultiplier/.test(game)&&/BunkrPrestige\?\.damageMultiplier/.test(game)&&/BunkrPrestige\?\.zombieMultiplier/.test(game),'core economy and combat need centralized Prestige multipliers');
...
for(const marker of ['js/systems/prestige.js?build=4','app.css?build=59','js/core/state.js?build=26','js/systems/operations.js?build=3','js/core/game.js?build=27'])assert(html.includes(marker),`cache marker missing: ${marker}`);
