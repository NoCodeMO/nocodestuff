...
function baseWindow(state){return{BunkrState:{get:()=>state,update:(reason,fn)=>{fn(state);return state},save(){},notify(){},offlineElapsedMs:0,bonus:()=>1},addEventListener(){},dispatchEvent(){}}}
...
const research=researchSandbox.window.BunkrResearch;
...
const expeditionWindow=baseWindow(expeditionState);expeditionWindow.BunkrGame={rates:()=>({coins:100,scrap:20,food:2,water:3})};expeditionWindow.BunkrMerchant={multiplier:resource=>resource==='coins'?5:1};
...
const expeditions=expeditionSandbox.window.BunkrExpeditions,cost=expeditions.deploymentCost(expeditionSandbox.window.BunkrConfig.EXPEDITIONS.store),coinRange=expeditions.rewardRange(expeditionSandbox.window.BunkrConfig.EXPEDITIONS.store,'coins');
...
const offlineWindow=baseWindow(offlineState);offlineWindow.BunkrGame={operationsSnapshot:()=>({rates:{coins:100,food:2,water:3,power:4,scrap:20,science:1},summary:{alerts:2,efficiency:.72}})};offlineWindow.BunkrMerchant={multiplier:resource=>resource==='coins'?5:1};
...
const offline=offlineSandbox.window.BunkrOffline.calculate(3600000);
...
console.log('Bunkr cross-system balance passed: dual-cost research, ration-funded expeditions, sustainable offline rates and scarce attention rewards.');
