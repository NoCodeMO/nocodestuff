...
for(const [pattern,message] of [
  ...
  [/\.enemyUnit\.horde\{right:0;bottom:18%;width:min\(61%,285px\);height:min\(64%,330px\)\}/,'horde container must stay anchored inside the stage'],
  [/\.enemyUnit\.horde \.enemySprite:nth-child\(2\)\{right:2%;bottom:22%!important;z-index:2;transform:scale\(\.86\)\}/,'middle horde member needs the safe depth preset'],
  [/\.enemyUnit\.horde \.enemySprite:nth-child\(3\)\{right:22%;bottom:44%!important;z-index:1;transform:scale\(\.76\)\}/,'rear horde member needs the safe depth preset'],
  [/\.enemyDeathUnit\.horde \.enemyDeathSprite:nth-child\(3\)\{right:22%;bottom:44%;z-index:1;transform:scale\(\.76\)\}/,'corpse formation must mirror the live formation'],
  [/\.enemyUnit\[data-rarity="brute"\]\{right:4px\}/,'brutes must retain a protected screen edge']
])if(!pattern.test(css))fail(message);
if(!/app\.css\?build=51/.test(html))fail('index must cache-bust the portrait stylesheet');
