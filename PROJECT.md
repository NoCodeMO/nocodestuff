# BUNKR: LAST SHELTER - DEVELOPER CONTEXT
...
### Core
- `js/core/brand.js` - canonical BUNKR identity, save-key constants and temporary one-release compatibility aliases for old public APIs/events.
...
- `js/core/room-progression.js` - pure permanent room Mastery/milestone chain shared by production, room cards, detail UI and regression probes.
...
- `scripts/room-progression-check.js` - permanent level-100 boundary behavior, cumulative Mastery bonuses, uncapped save normalization and finite extreme-level pricing.
...
- `scripts/research-network-check.js` and `scripts/research-network-probe.html` - all four branches, 17 project IDs, 97 levels, prerequisites, Prestige gates, save migration, timer/claim lifecycle, gameplay effects, plus real 390×844 portrait and 844×390 landscape geometry/lifecycle probes that verify the bottom Research action remains reachable.
...
1. brand and compatibility contract
2. config
3. economy
4. numbers
5. state
...
Do not casually reorder these. Prestige loads before every economy consumer so research, expeditions and the first game tick share its multipliers. Room operations loads before game because `BunkrGame.rates()` is the authoritative net-production calculation. Missions loads after game because it owns the custom Missions tab click handler. Visuals loads before game so it receives initial combat events. Expeditions and special rooms load before game so their APIs are available to the first render/economy tick.

## State: one source of truth

The production save key is `bunkr_last_shelter_v1` and the current schema is `schema: 20`. On first launch after the rebrand, `state.js` imports `afterlight_v4` without deleting it, then writes and verifies the canonical BUNKR save. `bunkr_v4` remains a short-lived pre-release fallback.

`window.BunkrState` owns the in-memory state and persistence. Systems must not independently read/write the main save through `localStorage`.
...
- `window.BunkrNumbers`
- `window.BunkrState`
- `window.BunkrPrestige`
- `window.BunkrOperations`
- `window.BunkrGame`
- `window.BunkrMissions`
- `window.BunkrExpeditions`
- `window.BunkrSpecialRooms`
- `window.BunkrResearch`
- `window.BunkrMerchant`
- `window.BunkrCommandCenter`
- `window.BunkrCodex`
- `window.BunkrCarePackage`
- `window.BunkrVisuals`
- `window.BunkrSurvivorDialogue`
- `window.BunkrAudio`
- `window.BunkrPlatform`

Compatibility globals used by inline UI:
- `buyRoom(id)`
- `buySpecialRoom(id)`
- `startBunkrExpedition(id)`
- `BunkrBonuses()`
- `BunkrSurvivors()`
...
- `bunkr:shot` - emitted by core combat; visuals/SFX can react.
- `bunkr:enemy-killed` - emitted once after coins/scrap and any Brute Core are granted; includes the exact reward and enemy metadata and owns the cash SFX trigger.
- `bunkr:enemy` - emitted when a new enemy or horde spawns; includes its rarity, art, in-game glow, HP, rewards and visual count.
- `bunkr:state` - important state mutation; `detail.reason` describes the change.
- `bunkr:survivors` - specialist roster changed.
- `bunkr:research-complete` - a persisted Research timer finished and the red claim notification became active.
- `bunkr:mission-claimed` - emitted after a successful claim; owns the mission reward fanfare.
- `bunkr:expedition-started` - emitted after Food/Water rations are atomically deducted and the sustainable reward rates are captured.
- `bunkr:expedition-complete` - emitted only after the scout has reached the target, searched it and followed the same route back to Bunkr; owns the expedition fanfare.
- `bunkr:expedition-roster` - the recovered specialist/companion roster changed after a returned haul was secured.
- `bunkr:merchant-purchase` - emitted after Uranium is spent and the boost is active; owns the Dealer celebration and fanfare.
- `bunkr:merchant-expired` - emitted when one or more persisted wall-clock boosts expire.
- `bunkr:merchant-free-activated` - emitted when a care-package jackpot activates a no-cost five-minute Dealer contract without spending Uranium.
- `bunkr:care-package-spawned` - emitted once when an economy-scaled drop begins descending.
- `bunkr:care-package-landed` - emitted once on road impact and owns the dedicated landing thud.
- `bunkr:care-package-opened` - emitted after its persisted reward is claimed and owns the long-form cache fanfare/reveal.
- `bunkr:care-package-missed` - emitted when the five-second claim window expires without a penalty.
- `bunkr:room-upgraded` - emitted after a successful normal-room upgrade; includes its exact level range, total cost and any newly reached milestone.
- `bunkr:dev-reward-claimed` - emitted once after a Command Center supply reward is safely added to the unified save; owns its fanfare.
- `bunkr:account` - local Commander login state changed.
- `bunkr:settings-changed` - visual accessibility settings changed and presentation systems should refresh.
- `bunkr:survivor-selected` - persisted survivor selection changed; visuals swap art and its configured muzzle anchor without replacing combat logic.
- `bunkr:survivor-dialogue-start` - a contextual/idle line begins; includes the survivor profile and dialogue context.
- `bunkr:survivor-dialogue-letter` - one audible typewriter character appears; audio owns the short, profile-specific retro bleep.
- `bunkr:survivor-dialogue-complete` - the complete line is visible and its hold timer has started.
- `bunkr:prestige-complete` - emitted only after a verified reset transaction; owns the survivor reveal, visuals swap and long Prestige fanfare.
- `bunkr:prestige-room` - a permanent Prestige Room was upgraded with Cores.
- `bunkr:prestige-contract` - all five current-cycle contracts were completed and their one bonus Core was claimed.

UI button sounds are handled centrally in `audio.js`; do not add per-button audio listeners. Combat inside `#scene` is excluded from the button handler and its gunshot is driven by `bunkr:shot`.

Example: future gun audio should listen to `bunkr:shot`, not attach another listener to the shoot button.
...
Special-room production is returned by `BunkrSpecialRooms.rates()` and integrated into the core economy. Do not add another production interval that writes resources independently.

Every built normal room participates in the operations network. The Generator is self-starting; the Purifier, Greenhouse and Living Quarters retain a small emergency recovery floor so a bad allocation cannot permanently soft-lock a run. Other rooms need configured combinations of Power, Water, Food, Scrap, Science and virtual Workforce. Living Quarters plus one starting survivor provide Workforce; Power/Water/Food/Scrap/Science use current production plus at most one five-minute stock reserve. When demand exceeds supply, Essential rooms receive resources before Normal and Low rooms. A paused room produces nothing and releases its load. Efficiency below 90% creates a visible room-card alert, while the room screen shows exact required/supplied rates, the bottleneck and a direct upgrade or priority recommendation. `BunkrGame.rates({useReserves:false,cache:false})` is the required sustainable-rate path for long-lived rewards: it ignores temporary stockpiles and does not overwrite the live UI snapshot.

Enemy encounters use one weighted table with an exact 100% total: Common 55%, Uncommon 25%, Rare 12%, Epic 5%, Legendary 2% and Brute 1%. The Common tier is split evenly between The Drifter and Zouki at 27.5% each; Dealer rarity boosts preserve that internal 50/50 split instead of doubling Common weight. Every non-Brute encounter has a 12% base horde chance, with a persisted pity counter guaranteeing a horde by the eighth eligible encounter. Brutes can never become hordes and neither advance nor reset that counter. A horde contains exactly three infected and grants exactly x3 HP, coins, scrap and kill credit compared with that same single infected.

The Drifter is the first completed character-specific death rollout. Its true-alpha horizontal sheet contains Impact, Collapse and Corpse frames on one shared ground baseline. Core combat includes the configured death asset in `bunkr:enemy-killed`; visuals snapshot the responsive encounter bounds, render the sequence in a separate corpse unit and hide only the defeated live unit. The final frame uses a wider source window because the horizontal corpse extends into the previous equal third; keep its vertical position at 52% so the complete corpse remains on the shared ground baseline. Do not restore a naive 300%/100% crop or the top/back of the skull will be clipped. The next encounter starts after the 390ms fall while the corpse remains for 1.5 seconds, so respawn never erases the corpse. Hordes create three synchronized frame players. At most three corpse units may coexist to keep rapid clicking safe. Future infected death sheets must reuse `deathAsset` and this renderer instead of introducing enemy-specific timers or kill listeners.

Every infected also owns a dedicated three-frame `hitAsset`. The Drifter and Zouki use the shared fully animated Common wrapper for idle breathing, their quick right-edge entrance and the 180ms shot reaction. Cinderback, Blue Shield, Bloater, Gilded Warden and Breaker keep their exact live WebP visible at rest and swap only during the shot window to normalized true-alpha 700x760 frames. The Breaker uses the same renderer with a 240ms heavy reaction and larger impact flash. Hordes animate all three visible members in sync, while one cleared timer prevents rapid shots from ending a newer hit animation early. `scripts/prepare-sprite-sheet.py` converts approved light-checker concepts into equal-cell RGBA sheets and supports explicit source ranges and targeted enclosed-background seeds for wide sprites.
...
Normal rooms use a smooth 1.142 price curve whose scale ramps from ×1 to ×125 across the opening levels, while output grows at 1.07 per level. The higher scale is what moves Prestige I from a short session into roughly a full passive day. Room levels have no designed cap. Every block of 100 levels adds another permanent Mastery Rank: absolute levels 5, 10, 25, 50, 75 and 100 grant x1.25, x1.5, x2, x3, x4 and x5; Mastery II continues at levels 105–200 and stacks up to x25, Mastery III continues at 205–300 and stacks up to x125, and the same chain expands indefinitely. Crossing 100, 200 or any later boundary never clears the displayed or economic milestone. The detail screen shows nearby absolute milestones rather than a resetting 0–100 list. Beyond the normal exponential number range, a finite 5e298 per-level price ceiling keeps legitimate upgrades possible without allowing Infinity or free purchases. Bulk x10 costs are the exact sum of sequential upgrades; MAX evaluates at most 500 levels and only buys those the current Coin balance can fully fund. Every quote is validated again inside the state transaction and all resources use a finite 1e300 ceiling.
...
All production, price and reward UI uses `BunkrNumbers`. Suffixes progress through K, M, B, T, Qa, Qi, Sx, Sp, Oc, No and Dc before continuing into higher tiers and scientific notation. Do not reintroduce local `fmt` implementations in individual systems.
...
- `enemy-common-drifter.webp`, `enemy-common-zouki.webp`, `enemy-uncommon-cinderback.webp`, `enemy-rare-blue-shield.webp`, `enemy-epic-bloater.webp`, `enemy-legendary-gilded-warden.webp`, `enemy-brute-breaker.webp` - transparent, left-facing enemy art with no baked or runtime rarity glow; rarity colors remain on the information card only
...
- `enemy-common-zouki-idle.png`, `enemy-common-zouki-walk.png`, `enemy-common-zouki-hit.png`, `enemy-common-zouki-death.png` - Zouki's complete true-alpha three-frame animation set. Live sheets use equal 700x760 cells; the death sheet uses 740x760 cells so her full horizontal corpse, bun and feet never clip.
...
- `enemy-uncommon-cinderback-hit.png`, `enemy-rare-blue-shield-hit.png`, `enemy-epic-bloater-hit.png`, `enemy-legendary-gilded-warden-hit.png`, `enemy-brute-breaker-hit.png` - normalized true-alpha three-frame shot-shock sheets on one shared baseline; the original live WebPs remain the idle/spawn source
...
- Every successful shot now emits a capped, true DOM pixel-blood burst through the existing `bunkr:shot` event. Each infected type has its own blood/accent palette, particle weight and recoil animation; Blue Shield and the Gilded Warden mix restrained armor sparks into their blood. The renderer hard-caps live particles at 48 and disables them for reduced-motion/reduced-effects players.
...
- All seven infected characters have complete three-frame death/corpse sequences. The Drifter and Zouki also have full three-frame idle breathing, entrance walk/run and shot-shock animation; walk/idle rollout for the remaining five infected is intentionally deferred.
