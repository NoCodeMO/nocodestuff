...
Every infected also owns a dedicated three-frame `hitAsset`. The Drifter reuses its existing animated wrapper; Cinderback, Blue Shield, Bloater, Gilded Warden and Breaker keep their exact live WebP visible at rest and swap only during the shared 180ms shot window to normalized true-alpha 700x760 frames. The Breaker uses the same renderer with a 240ms heavy reaction and larger impact flash. Hordes animate all three visible members in sync, while one cleared timer prevents rapid shots from ending a newer hit animation early. `scripts/prepare-sprite-sheet.py` converts approved light-checker concepts into equal-cell RGBA sheets and supports explicit source ranges and targeted enclosed-background seeds for wide sprites.
...
- `enemy-common-drifter.webp`, `enemy-uncommon-cinderback.webp`, `enemy-rare-blue-shield.webp`, `enemy-epic-bloater.webp`, `enemy-legendary-gilded-warden.webp`, `enemy-brute-breaker.webp` - transparent, left-facing enemy art with no baked or runtime rarity glow; rarity colors remain on the information card only
