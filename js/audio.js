...
function tone(frequency,duration=.055,volume=.07,delay=0,type='sine',endFrequency=frequency){
  const level=channelLevel();if(!level)return false;
  const ctx=unlockUi();if(!ctx)return false;
  const start=ctx.currentTime+delay,osc=ctx.createOscillator(),gain=ctx.createGain();
  osc.type=type;osc.frequency.setValueAtTime(frequency,start);osc.frequency.exponentialRampToValueAtTime(Math.max(20,endFrequency),start+duration);gain.gain.setValueAtTime(.0001,start);gain.gain.exponentialRampToValueAtTime(volume*level,start+.004);gain.gain.exponentialRampToValueAtTime(.0001,start+duration);
...
function noise(duration=.09,volume=.16,delay=0,cutoff=1800){
  const level=channelLevel();if(!level)return false;const ctx=unlockUi();if(!ctx)return false;const length=Math.ceil(ctx.sampleRate*duration),buffer=ctx.createBuffer(1,length,ctx.sampleRate),data=buffer.getChannelData(0);for(let i=0;i<length;i++)data[i]=(Math.random()*2-1)*(1-i/length);
  const start=ctx.currentTime+delay,source=ctx.createBufferSource(),filter=ctx.createBiquadFilter(),gain=ctx.createGain();source.buffer=buffer;filter.type='lowpass';filter.frequency.value=cutoff;gain.gain.setValueAtTime(volume*level,start);gain.gain.exponentialRampToValueAtTime(.0001,start+duration);source.connect(filter).connect(gain).connect(master);source.start(start);return true;
}
const UI_VOLUME=1.1;
...
function rewardSound(kind='mission'){
  return channelSound('reward',()=>{if(kind==='expedition'){tone(130,.32,.11,0,'triangle',195);tone(392,.28,.075,.12,'sine',523);tone(523,.3,.075,.3,'sine',659);tone(659,.38,.08,.48,'sine',880);tone(988,.46,.055,.68,'sine',1320);noise(.7,.025,.18,5200);return true}
...
function researchSound(){return channelSound('reward',()=>{tone(262,.22,.07,0,'sine',392);tone(392,.26,.075,.14,'triangle',523);tone(523,.3,.07,.3,'sine',784);tone(1047,.42,.05,.48,'sine',1319);noise(.42,.018,.22,6200);return true})}
function merchantSound(){return channelSound('reward',()=>{tone(110,.2,.1,0,'sawtooth',165);noise(.16,.045,.02,900);tone(440,.16,.08,.1,'triangle',660);tone(660,.2,.075,.2,'sine',990);tone(990,.28,.065,.34,'sine',1480);tone(1480,.38,.045,.52,'sine',1975);noise(.52,.018,.22,6500);return true})}
function carePackageLandSound(){return channelSound('reward',()=>{noise(.2,.16,0,720);tone(72,.22,.15,0,'sine',44);tone(185,.24,.055,.045,'triangle',112);noise(.08,.045,.12,2600);return true})}
function carePackageOpenSound(){return channelSound('reward',()=>{noise(.09,.07,0,2100);tone(180,.12,.09,0,'square',260);tone(392,.18,.075,.08,'triangle',523);tone(523,.2,.075,.19,'sine',698);tone(698,.24,.07,.31,'sine',988);tone(1047,.34,.06,.47,'sine',1480);tone(1568,.45,.045,.67,'sine',2093);noise(.6,.022,.2,6200);return true})}
function roomMilestoneSound(event){if(!event.detail?.milestone)return false;return channelSound('reward',()=>{tone(196,.18,.07,0,'triangle',294);tone(392,.2,.065,.1,'sine',523);tone(587,.24,.065,.22,'sine',784);tone(988,.34,.05,.36,'sine',1319);noise(.32,.014,.18,5200);return true})}
function prestigeSound(){return channelSound('reward',()=>{tone(82,.65,.12,0,'sawtooth',123);noise(.42,.055,.02,1150);tone(196,.45,.09,.18,'triangle',294);tone(392,.5,.085,.48,'sine',523);tone(587,.55,.08,.76,'sine',784);tone(784,.62,.075,1.05,'sine',1175);tone(1175,.78,.06,1.38,'sine',1760);tone(1760,.9,.045,1.75,'sine',2349);noise(1.1,.018,.48,6800);return true})}
...
function preview(channel){unlockUi();if(channel==='music')return enabled?play():false;if(!effectsEnabled)return false;if(channel==='combat'){lastShot=0;return gunshot()}if(channel==='reward')return rewardSound('mission');return uiSound('confirm')}
