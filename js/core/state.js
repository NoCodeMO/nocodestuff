...
const defaults=()=>({
  ...
  settings:{music:true,uiSfx:true,musicVolume:1,uiVolume:1,combatVolume:1,rewardVolume:1,reducedEffects:false},
  last:Date.now()
});
...
function normalize(){
  ...
  state.command={...base.command,...(state.command||{}),account:{...base.command.account,...(state.command?.account||{})}};state.command.read=Array.isArray(state.command.read)?state.command.read:[];state.command.claimed=Array.isArray(state.command.claimed)?state.command.claimed:[];state.settings={...base.settings,...(state.settings||{})};for(const key of ['musicVolume','uiVolume','combatVolume','rewardVolume']){const value=Number(state.settings[key]);state.settings[key]=Number.isFinite(value)?Math.max(0,Math.min(1,value)):1}
