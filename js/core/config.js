(()=>{'use strict';
const ROOM_ECONOMY=Object.freeze({costGrowth:1.142,costScale:125,costScaleRamp:25,rateGrowth:1.07,masterySize:100,maximumBulk:500,unlimitedRoomLevels:true,maximumRoomCost:5e298,maximumValue:1e300,bunkerLevelEvery:4});
...
const RESEARCH_BRANCHES=Object.freeze([
  Object.freeze({id:'survival',icon:'◒',name:'SURVIVAL SYSTEMS',short:'SURVIVAL',description:'Close the Food and Water loop so every room can keep operating.'}),
  Object.freeze({id:'industry',icon:'⚙',name:'POWER & INDUSTRY',short:'INDUSTRY',description:'Turn salvage, power and automation into a stronger bunker economy.'}),
  Object.freeze({id:'combat',icon:'▥',name:'COMBAT ENGINEERING',short:'COMBAT',description:'Build precise weapons doctrine for hordes, Brutes and the road.'}),
  Object.freeze({id:'field',icon:'⌖',name:'FIELD OPERATIONS',short:'FIELD OPS',description:'Shorten routes and recover more from every expedition.'})
]);
const RESEARCH=[
  {id:'filters',branch:'survival',tier:1,icon:'💧',name:'WATER RECYCLING',desc:'Industrial reclaim loops improve Water production.',benefit:'+15% WATER PRODUCTION PER LEVEL',baseCost:11,scienceCost:3,baseSeconds:32,costGrowth:1.57,scienceGrowth:1.46,timeGrowth:1.29,effect:1.15,maxLevel:8,requires:[]},
  {id:'hydro',branch:'survival',tier:2,icon:'🌿',name:'HYDROPONICS',desc:'Closed growing racks make every harvest more productive.',benefit:'+15% FOOD PRODUCTION PER LEVEL',baseCost:10,scienceCost:3,baseSeconds:30,costGrowth:1.57,scienceGrowth:1.46,timeGrowth:1.29,effect:1.15,maxLevel:8,requires:[{id:'filters',level:1}]},
  {id:'closedLoop',branch:'survival',tier:3,icon:'♻',name:'CLOSED-LOOP ECOLOGY',desc:'Shared recycling systems reinforce both essential supply chains.',benefit:'+6% FOOD & WATER PRODUCTION PER LEVEL',baseCost:260,scienceCost:95,baseSeconds:900,costGrowth:1.72,scienceGrowth:1.62,timeGrowth:1.42,effect:1.06,maxLevel:5,requires:[{id:'hydro',level:2}]},
  {id:'archiveProtocol',branch:'survival',tier:4,icon:'▣',name:'ARCHIVE PROTOCOL',desc:'Recovered research procedures shorten every future experiment.',benefit:'-8% RESEARCH TIME PER LEVEL',baseCost:1700,scienceCost:720,baseSeconds:3600,costGrowth:1.82,scienceGrowth:1.7,timeGrowth:1.48,effect:.92,maxLevel:3,prestige:1,requires:[{id:'closedLoop',level:2}]},

  {id:'circuit',branch:'industry',tier:1,icon:'▦',name:'CIRCUIT SALVAGE',desc:'Sorting usable electronics raises Workshop Scrap output.',benefit:'+12% SCRAP PRODUCTION PER LEVEL',baseCost:9,scienceCost:3,baseSeconds:28,costGrowth:1.58,scienceGrowth:1.47,timeGrowth:1.3,effect:1.12,maxLevel:8,requires:[]},
  {id:'solar',branch:'industry',tier:2,icon:'⚡',name:'EFFICIENT GENERATORS',desc:'A tuned microgrid strengthens all passive bunker production.',benefit:'+8% ALL PASSIVE PRODUCTION PER LEVEL',baseCost:12,scienceCost:4,baseSeconds:35,costGrowth:1.6,scienceGrowth:1.48,timeGrowth:1.31,effect:1.08,maxLevel:8,requires:[{id:'circuit',level:1}]},
  {id:'automation',branch:'industry',tier:3,icon:'⌘',name:'WORKSHOP AUTOMATION',desc:'Automated trading and fabrication improve passive Coin output.',benefit:'+10% PASSIVE COIN PRODUCTION PER LEVEL',baseCost:16,scienceCost:6,baseSeconds:45,costGrowth:1.61,scienceGrowth:1.5,timeGrowth:1.32,effect:1.1,maxLevel:8,requires:[{id:'solar',level:2}]},
  {id:'fortressGrid',branch:'industry',tier:4,icon:'⬡',name:'FORTRESS GRID',desc:'Standardized construction plans reduce every normal room cost.',benefit:'-6% NORMAL ROOM COST PER LEVEL',baseCost:4800,scienceCost:1900,baseSeconds:5400,costGrowth:1.86,scienceGrowth:1.74,timeGrowth:1.5,effect:.94,maxLevel:3,prestige:3,requires:[{id:'automation',level:3}]},

  {id:'tools',branch:'combat',tier:1,icon:'▰',name:'BALLISTICS',desc:'Calibrated weapons deliver more damage with every manual shot.',benefit:'+18% MANUAL DAMAGE PER LEVEL',baseCost:8,scienceCost:2,baseSeconds:25,costGrowth:1.58,scienceGrowth:1.46,timeGrowth:1.3,effect:1.18,maxLevel:8,requires:[]},
  {id:'armorPiercing',branch:'combat',tier:2,icon:'◈',name:'ARMOR PIERCING',desc:'Hardened ammunition creates more critical-hit opportunities.',benefit:'+1.5% CRITICAL CHANCE PER LEVEL',baseCost:40,scienceCost:14,baseSeconds:120,costGrowth:1.64,scienceGrowth:1.54,timeGrowth:1.35,effect:.015,maxLevel:5,requires:[{id:'tools',level:2}]},
  {id:'hordeControl',branch:'combat',tier:3,icon:'▥',name:'HORDE CONTROL',desc:'Controlled fire suppresses clustered infected before they advance.',benefit:'+25% DAMAGE AGAINST HORDES PER LEVEL',baseCost:220,scienceCost:80,baseSeconds:720,costGrowth:1.72,scienceGrowth:1.62,timeGrowth:1.42,effect:.25,maxLevel:3,requires:[{id:'armorPiercing',level:2}]},
  {id:'walls',branch:'combat',tier:4,icon:'▤',name:'BRUTE BREAKER',desc:'Heavy-target doctrine improves infected bounties and Brute damage.',benefit:'+12% INFECTED BOUNTY & +15% BRUTE DAMAGE PER LEVEL',baseCost:14,scienceCost:5,baseSeconds:40,costGrowth:1.59,scienceGrowth:1.49,timeGrowth:1.31,effect:1.12,secondaryEffect:.15,maxLevel:8,requires:[{id:'hordeControl',level:1}]},

  {id:'routeMapping',branch:'field',tier:1,icon:'⌖',name:'ROUTE MAPPING',desc:'Reliable field maps increase the value of recovered Coin caches.',benefit:'+8% EXPEDITION COINS PER LEVEL',baseCost:18,scienceCost:7,baseSeconds:55,costGrowth:1.61,scienceGrowth:1.5,timeGrowth:1.33,effect:1.08,maxLevel:6,requires:[]},
  {id:'rapidDeployment',branch:'field',tier:2,icon:'➤',name:'RAPID DEPLOYMENT',desc:'Pre-packed kits shorten every outbound and return journey.',benefit:'-6% EXPEDITION TIME PER LEVEL',baseCost:75,scienceCost:28,baseSeconds:240,costGrowth:1.67,scienceGrowth:1.57,timeGrowth:1.37,effect:.94,maxLevel:5,requires:[{id:'routeMapping',level:1}]},
  {id:'advancedRecovery',branch:'field',tier:3,icon:'▣',name:'ADVANCED RECOVERY',desc:'Field sorting protects valuable Scrap on the trip home.',benefit:'+10% EXPEDITION SCRAP PER LEVEL',baseCost:320,scienceCost:120,baseSeconds:1200,costGrowth:1.74,scienceGrowth:1.64,timeGrowth:1.43,effect:1.1,maxLevel:5,requires:[{id:'rapidDeployment',level:2}]},
  {id:'companionRescue',branch:'field',tier:4,icon:'♟',name:'COMPANION RESCUE',desc:'Specialized signals make undiscovered companions easier to locate.',benefit:'+1.5% COMPANION FIND CHANCE PER LEVEL',baseCost:1250,scienceCost:520,baseSeconds:3000,costGrowth:1.8,scienceGrowth:1.69,timeGrowth:1.47,effect:.015,maxLevel:3,prestige:2,requires:[{id:'advancedRecovery',level:2}]},
  {id:'geneticArchive',branch:'field',tier:5,icon:'✦',name:'GENETIC ARCHIVE',desc:'A living recovery index strengthens companion and specialist signals.',benefit:'+2% COMPANION & +4% SPECIALIST CHANCE PER LEVEL',baseCost:9800,scienceCost:4200,baseSeconds:7200,costGrowth:1.9,scienceGrowth:1.78,timeGrowth:1.52,effect:.02,secondaryEffect:.04,maxLevel:3,prestige:5,requires:[{id:'companionRescue',level:3}]}
];
const EXPEDITIONS={
...
const ENEMIES=[
  {id:'common',tier:'common',tierShare:.5,rarity:'COMMON',name:'THE DRIFTER',chance:27.5,asset:'assets/enemy-common-drifter.webp',idleAsset:'assets/enemy-common-drifter-idle.png',walkAsset:'assets/enemy-common-drifter-walk.png',hitAsset:'assets/enemy-common-drifter-hit.png',deathAsset:'assets/enemy-common-drifter-death.png',hpMultiplier:1,bountyMultiplier:1,scrapMultiplier:1,accent:'#b7b4a5'},
  {id:'zouki',tier:'common',tierShare:.5,rarity:'COMMON',name:'ZOUKI',chance:27.5,asset:'assets/enemy-common-zouki.webp',idleAsset:'assets/enemy-common-zouki-idle.png',walkAsset:'assets/enemy-common-zouki-walk.png',hitAsset:'assets/enemy-common-zouki-hit.png',deathAsset:'assets/enemy-common-zouki-death.png',hpMultiplier:1,bountyMultiplier:1,scrapMultiplier:1,accent:'#b7b4a5'},
...
window.BunkrConfig=Object.freeze({ROOMS,ROOM_ECONOMY,ROOM_MILESTONES,RESEARCH_BRANCHES,RESEARCH,EXPEDITIONS,SPECIALISTS,EXPEDITION_COMPANIONS,SPECIAL_ROOMS,MERCHANT_OFFERS,SURVIVOR_SKINS,PRESTIGE,SURVIVOR_DIALOGUE,COMMAND_MESSAGES,ENEMIES,COMBAT,CARE_PACKAGE,OFFLINE,OPERATIONS});
})();
