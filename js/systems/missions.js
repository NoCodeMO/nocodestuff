...
const LEGACY_MISSIONS=[['k5','Kill 5 infected','kills',5,'coins',75,1],['g2','Generator Level 2','generator',2,'coins',120,1],['w1','Build the Workshop','workshop',1,'scrap',15,1],['b2','Reach Bunker Level 2','bunker',2,'coinMult',1.05,1],['k15','Kill 15 infected','kills',15,'coins',300,2],['g5','Generator Level 5','generator',5,'powerMult',1.1,2],['w5','Workshop Level 5','workshop',5,'scrapMult',1.1,2],['gh1','Build the Greenhouse','greenhouse',1,'food',100,2],['b3','Reach Bunker Level 3','bunker',3,'prodMult',1.05,2],['k50','Kill 50 infected','kills',50,'zombieMult',1.1,3],['g10','Generator Level 10','generator',10,'powerMult',1.15,3],['w10','Workshop Level 10','workshop',10,'scrapMult',1.15,3],['gh5','Greenhouse Level 5','greenhouse',5,'foodMult',1.1,3],['p5','Water Purifier Level 5','purifier',5,'waterMult',1.1,3],['b5','Reach Bunker Level 5','bunker',5,'prodMult',1.06,3],['k100','Kill 100 infected','kills',100,'damageMult',1.1,4],['lab5','Research Lab Level 5','lab',5,'scienceMult',1.1,4],['g25','Generator Level 25','generator',25,'prodMult',1.06,4],['w25','Workshop Level 25','workshop',25,'costMult',.97,4],['b7','Reach Bunker Level 7','bunker',7,'offlineMult',1.1,4],['k250','Kill 250 infected','kills',250,'zombieMult',1.12,5],['gh10','Greenhouse Level 10','greenhouse',10,'foodMult',1.15,5],['p10','Water Purifier Level 10','purifier',10,'waterMult',1.15,5],['l10','Living Quarters Level 10','living',10,'coinMult',1.08,5],['b10','Reach Bunker Level 10','bunker',10,'prodMult',1.08,5],['boss10','Defeat 10 Brutes','bosses',10,'damageMult',1.12,6],['lab10','Research Lab Level 10','lab',10,'scienceMult',1.15,6],['s10','Storage Level 10','storage',10,'prodMult',1.05,6],['t10','Auto Turret Level 10','turret',10,'damageMult',1.12,6],['b15','Reach Bunker Level 15','bunker',15,'zombieMult',1.15,6],['k500','Kill 500 infected','kills',500,'damageMult',1.15,7],['g50','Generator Level 50','generator',50,'powerMult',1.25,7],['w50','Workshop Level 50','workshop',50,'prodMult',1.08,7],['gh25','Greenhouse Level 25','greenhouse',25,'foodMult',1.2,7],['p25','Water Purifier Level 25','purifier',25,'waterMult',1.2,7],['b20','Reach Bunker Level 20','bunker',20,'prodMult',1.1,7],['k1000','Kill 1,000 infected','kills',1000,'zombieMult',1.18,8],['lab25','Research Lab Level 25','lab',25,'scienceMult',1.2,8],['s25','Storage Level 25','storage',25,'costMult',.94,8],['t25','Auto Turret Level 25','turret',25,'damageMult',1.18,8],['b25','Reach Bunker Level 25','bunker',25,'offlineMult',1.15,8],['boss50','Defeat 50 Brutes','bosses',50,'zombieMult',1.2,9],['l25','Living Quarters Level 25','living',25,'coinMult',1.12,9],['rooms250','Reach 250 combined room levels','roomtotal',250,'prodMult',1.1,9],['k5000','Kill 5,000 infected','kills',5000,'damageMult',1.2,9],['b50','Reach Bunker Level 50','bunker',50,'prodMult',1.15,10],['rooms500','Reach 500 combined room levels','roomtotal',500,'costMult',.9,10],['boss100','Defeat 100 Brutes','bosses',100,'zombieMult',1.25,10],['k25000','Kill 25,000 infected','kills',25000,'zombieMult',1.25,10],['master','Reach Bunker Level 100','bunker',100,'prodMult',1.2,10]];

const CHAPTER_NAMES=['PERIMETER WATCH','RUST BELT','ASH DISTRICT','NIGHT SIGNAL','DEAD HIGHWAY','IRON QUARANTINE','BLACK RAIN','SILENT METRO','RED HORIZON','BROKEN CITADEL','FALLOUT FRONTIER','VOID SECTOR','LAST FREQUENCY','OMEGA TERRITORY','BUNKR ETERNAL'];
...
const BONUS_LIMITS=Object.freeze({prodMult:2.5,coinMult:1.75,powerMult:2.5,scrapMult:2.5,foodMult:2.5,waterMult:2.5,scienceMult:2.5,damageMult:3,zombieMult:3,offlineMult:2,costMult:.75});
const bonusTypes=new Set(Object.keys(BONUS_LIMITS));
function boundedBonus(type,current,value){const next=(Number(current)||1)*(Number(value)||1);return type==='costMult'?Math.max(BONUS_LIMITS.costMult,next):Math.min(BONUS_LIMITS[type]||3,next)}
...
function grant(q,state){const crystals=uraniumReward(q),payout=cachePayout(q),label=reward(q,payout);M.claimed.push(q[0]);state.uranium=(state.uranium||0)+crystals;state.stats.uraniumEarned=(state.stats.uraniumEarned||0)+crystals;if(payout){state[payout.resource]=(state[payout.resource]||0)+payout.amount;if(payout.resource==='coins')state.total=(state.total||0)+payout.amount}else if(['coins','scrap','food'].includes(q[4])){state[q[4]]=(state[q[4]]||0)+q[5];if(q[4]==='coins')state.total=(state.total||0)+q[5]}else if(bonusTypes.has(q[4]))M.bonuses[q[4]]=boundedBonus(q[4],M.bonuses[q[4]],q[5]);return{q,crystals,payout,label}}
function claim(q){
  if(!q||M.claimed.includes(q[0])||val(q)<q[3])return false;
  let result;ST.update('mission',state=>{result=grant(q,state)});
  window.dispatchEvent(new CustomEvent('bunkr:mission-claimed',{detail:{id:q[0],reward:result.label,uranium:result.crystals,batch:false}}));celebrate(q,result.label);render();window.BunkrGame?.hud?.();window.BunkrGame?.renderRooms?.();return true;
}
...
if((Number(M.balanceVersion)||0)<1)ST.update('mission-balance-v1',state=>{state.missions.bonuses=rebuiltBonuses(state.missions.claimed);state.missions.balanceVersion=1});
...
window.BunkrBonuses=()=>({...M.bonuses});window.BunkrMissions={render,claim,claimAll,all:()=>Q.map(mission=>[...mission]),crystalReward:mission=>uraniumReward(mission),progress:()=>({complete:M.claimed.length,total:Q.length})};
...
window.addEventListener('bunkr:state',render);setInterval(render,1000);render();
})();
