...
function claim(q){
  ...
  const crystals=uraniumReward(q),payout=cachePayout(q),label=reward(q,payout);
  ST.update('mission',state=>{M.claimed.push(q[0]);state.uranium=(state.uranium||0)+crystals;state.stats.uraniumEarned=(state.stats.uraniumEarned||0)+crystals;if(payout){state[payout.resource]=(state[payout.resource]||0)+payout.amount;if(payout.resource==='coins')state.total=(state.total||0)+payout.amount}else if(['coins','scrap','food'].includes(q[4])){state[q[4]]=(state[q[4]]||0)+q[5];if(q[4]==='coins')state.total=(state.total||0)+q[5]}else M.bonuses[q[4]]=(M.bonuses[q[4]]||1)*q[5]});
  window.dispatchEvent(new CustomEvent('afterlight:mission-claimed',{detail:{id:q[0],reward:label,uranium:crystals}}));celebrate(q,label);render();window.AfterlightGame?.hud?.();window.AfterlightGame?.renderRooms?.();return true;
}
function render(){
  ...
  box.dataset.missionCount=String(Q.length);box.innerHTML=`<div class="missionPanel"><header><b>MISSION LOG</b><small>${complete}/${Q.length} COMPLETE</small><button id="missionClose">×</button></header><div class="missionCampaign"><span><b>200 AFTERLIGHT DIRECTIVES</b><small>15 NEW OPERATIONS CHAPTERS · 150 NEW MISSIONS</small></span><div><i style="width:${progress}%"></i></div></div>${qs.map(q=>{const value=Math.min(q[3],val(q)),done=value>=q[3],crystals=uraniumReward(q);return `<article class="${done?'done':''}"><em>${q[7]||'FOUNDATION'}</em><b>${q[1]}</b><small>${fmt(value)} / ${fmt(q[3])}</small><div><i style="width:${Math.min(100,value/q[3]*100)}%"></i></div><span>${reward(q)} <em>◆ +${crystals}</em></span>${done?`<button data-q="${q[0]}">CLAIM</button>`:''}</article>`}).join('')||'<p>Raise your Bunker Level to discover new missions.</p>'}</div>`;
  box.querySelector('#missionClose').onclick=()=>box.classList.remove('open');box.querySelectorAll('[data-q]').forEach(button=>button.onclick=()=>claim(Q.find(q=>q[0]===button.dataset.q)));
}
...
window.AfterlightBonuses=()=>({...M.bonuses});window.AfterlightMissions={render,claim,all:()=>Q.map(mission=>[...mission]),crystalReward:mission=>uraniumReward(mission),progress:()=>({complete:M.claimed.length,total:Q.length})};
