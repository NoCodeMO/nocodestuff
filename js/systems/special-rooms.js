(()=>{'use strict';
const CFG=window.BunkrConfig,ST=window.BunkrState,NUM=window.BunkrNumbers;if(!CFG||!ST||!NUM)throw new Error('Special room dependencies missing');
const S=ST.get(),ROOMS=CFG.SPECIAL_ROOMS;
const found=()=>window.BunkrExpeditions?.survivors?.()||S.expeditions?.survivors||[];
...
const cost=r=>Math.max(1,Math.floor(r.baseCost*Math.pow(1.7,level(r))*ST.bonus('costMult')*(window.BunkrPrestige?.roomCostMultiplier?.()||1)));
...
function buy(id){const r=ROOMS.find(x=>x.id===id);if(!r||!unlocked(r))return false;const price=cost(r);if(S.coins<price)return false;ST.update('special-room',s=>{s.coins-=price;s.specialRooms[id]=(s.specialRooms[id]||0)+1});renderAll();window.BunkrGame?.hud?.();return true}
...
window.buySpecialRoom=buy;window.BunkrSpecialRooms={buy,rates,renderMain,renderBuild,renderAll,levels:()=>({...S.specialRooms})};
window.addEventListener('bunkr:survivors',renderAll);window.addEventListener('bunkr:state',e=>{if(['room','special-room','expedition'].includes(e.detail?.reason))renderAll()});setTimeout(renderAll,150);
})();
