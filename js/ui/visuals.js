...
window.addEventListener('afterlight:shot',shotFeedback);window.addEventListener('afterlight:enemy',e=>{const d=e.detail||{},tier=d.boss?5:Math.min(5,1+Math.floor((d.kills||0)/10));enemy.dataset.tier=String(tier);restart(enemy,'spawn');document.getElementById('enemyCard')?.classList.toggle('bossGlow',!!d.boss)});
...
window.AfterlightVisuals={applyRoomArt:apply,shotFeedback};
})();