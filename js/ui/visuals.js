...
function damageFloat(event){const detail=event.detail||{},damage=Math.max(0,Number(detail.damage)||0);if(!damage)return;const stageRect=stage.getBoundingClientRect(),enemyRect=enemyUnit.getBoundingClientRect();if(!stageRect.width||!enemyRect.width)return;const float=document.createElement('b'),offsets=[-.14,.08,-.04,.16,-.1,0];float.className='damageNumber';float.setAttribute('aria-hidden','true');float.textContent='-'+NUM.format(damage);float.style.left=(enemyRect.left-stageRect.left+enemyRect.width*(.5+offsets[damageFloatIndex%offsets.length]))+'px';float.style.top=(enemyRect.top-stageRect.top+enemyRect.height*.28)+'px';damageFloatIndex+=1;stage.append(float);const active=stage.querySelectorAll('.damageNumber');if(active.length>8)active[0].remove();setTimeout(()=>float.remove(),760)}
function shotFeedback(event){clearSpawn();restart(survivorUnit,'shoot');restart(enemyUnit,'hit');restart(muzzle,'fire');restart(impact,'pop');damageFloat(event);setTimeout(()=>{survivorUnit.classList.remove('shoot');enemyUnit.classList.remove('hit')},180)}
...
window.AfterlightVisuals={shotFeedback,damageFloat,killRewards,renderEnemy,enemyUnit:()=>enemyUnit,survivorUnit:()=>survivorUnit,parallaxLayers:()=>WORLD_LAYERS.map(([name,src])=>({name,src}))};
})();
